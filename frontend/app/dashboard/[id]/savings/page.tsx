...
export default function SavingsPage() {
	...
	const savingsTransactions = useGetTransactions({
		userId: params.id as string,
		type: "saving",
		size: 100 // Get all savings transactions
	});
			...
			<main className="flex-1 w-full max-w-4xl mx-auto px-6 py-10 flex flex-col gap-6">
				...
				<section>
					<div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-6">
						...
						{Array.isArray(savingsTransactions) && savingsTransactions.length > 0 ? (
							...
							<div className="text-center py-8 text-slate-500 dark:text-slate-400">
								...
								<Button
									text="Add Savings"
									onClick={() => router.push(`/dashboard/${params.id}/save`)}
									className="mt-4"
								/>
							</div>
						)}
